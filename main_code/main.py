...
if __name__ == "__main__":
    ...
    wf_completed = 0
    t = 0
    while workflow_id != wf_completed or wf_completed == 0:
        for i in range (0, edge_num + 1):
            
            if t < receive_t and i != 0:
            ...
            for func in cluster.server_pool[i].executing_queue:
                if func.finish_time <= t:       # 函数执行完毕
                    ...
                    if i != func.belong_server:
                        func.finish_time += func.funcTransTime(i, cluster.dists, 1)
                        cluster.server_pool[i].trans_queue.append(func)
                    else:
            ...
            # 遍历传输队列
            temp_trans = []
            for func in cluster.server_pool[i].trans_queue:
                if func.finish_time <= t:
                    cluster.server_pool[func.belong_server].workflow_queue[func.belong_wf].executing_tasks[func.belong_task].functions_left -= 1
                else:
                    temp_trans.append(func)
            cluster.server_pool[i].trans_queue = temp_trans
